...
class TestSheetGenerator {
  ...
  async generateCSV(results, reportDir) {
    ...
    const csvData = results.map(result => {
      ...
      return {
        ...
        test_category: this.cleanStringForCSV(testCase?.test_category || result.test_category || '未分類'),
        test_case_name: this.cleanStringForCSV(testCase?.test_case_name || result.test_name),
        priority: testCase?.priority || result.priority || 'Medium',
        preconditions: this.cleanStringForCSV(testCase?.preconditions || ''),
        test_steps: this.formatTestSteps(testCase?.test_steps),
        expected_result: this.cleanStringForCSV(testCase?.expected_result || ''),
        actual_result: this.cleanStringForCSV(result.actual_result || ''),
    ...
    // エンコーディング設定を取得
    const primaryEncoding = this.config.output_settings?.encoding?.primary || 'shift_jis';
    const secondaryEncoding = this.config.output_settings?.encoding?.secondary || 'utf8';
    
    // プライマリエンコーディングでの保存
    if (primaryEncoding === 'shift_jis') {
      // Shift-JISをメインにする
      const sjisBuffer = iconv.encode(utf8Content, 'Shift_JIS');
      fs.writeFileSync(csvPath, sjisBuffer);
      console.log(chalk.green(`✅ CSV生成完了: test-results.csv (Shift-JIS)`));
      
      // UTF-8版も追加で保存
      if (secondaryEncoding === 'utf8') {
        const BOM = '\uFEFF';
        const utf8Path = path.join(path.dirname(csvPath), 'test-results_utf8.csv');
        fs.writeFileSync(utf8Path, BOM + utf8Content, 'utf8');
        console.log(chalk.green(`   追加: test-results_utf8.csv (UTF-8 BOM付き)`));
      }
    } else {
      // UTF-8をメインにする（BOM付き）
      const BOM = '\uFEFF';
      fs.writeFileSync(csvPath, BOM + utf8Content, 'utf8');
      console.log(chalk.green(`✅ CSV生成完了: test-results.csv (UTF-8 BOM付き)`));
      
      // Shift-JIS版も追加で保存
      if (secondaryEncoding === 'shift_jis') {
        const sjisBuffer = iconv.encode(utf8Content, 'Shift_JIS');
        const sjisPath = path.join(path.dirname(csvPath), 'test-results_sjis.csv');
        fs.writeFileSync(sjisPath, sjisBuffer);
        console.log(chalk.green(`   追加: test-results_sjis.csv (Shift-JIS)`));
      }
    }
  }
  ...
  async generateExcel(results, reportDir) {
    ...
    if (primaryEncoding === 'shift_jis') {
      ...
      XLSX.writeFile(workbook, excelPath, { 
        bookType: 'xlsx',
        codepage: 932  // Shift-JISのコードページ
      });
      console.log(chalk.green(`✅ Excel生成完了: test-results.xlsx (Shift-JIS対応)`));
      
      // UTF-8版も追加で保存
      if (secondaryEncoding === 'utf8') {
        const utf8ExcelPath = path.join(reportDir, 'test-results_utf8.xlsx');
        XLSX.writeFile(workbook, utf8ExcelPath);
        console.log(chalk.green(`   追加: test-results_utf8.xlsx (UTF-8)`));
      }
    } else {
      // UTF-8版をメインにする
      XLSX.writeFile(workbook, excelPath);
      console.log(chalk.green(`✅ Excel生成完了: test-results.xlsx (UTF-8)`));
      
      // Shift-JIS対応版も追加で保存
      if (secondaryEncoding === 'shift_jis') {
        const sjisExcelPath = path.join(reportDir, 'test-results_sjis.xlsx');
        XLSX.writeFile(workbook, sjisExcelPath, { 
          bookType: 'xlsx',
          codepage: 932  // Shift-JISのコードページ
        });
        console.log(chalk.green(`   追加: test-results_sjis.xlsx (Shift-JIS対応)`));
      }
    }
  }
  ...
  prepareResultsData(results) {
    return results.map(result => {
      ...
      return {
        ...
        'テストカテゴリ': this.cleanStringForCSV(testCase?.test_category || result.test_category || '未分類'),
        'テスト項目名': this.cleanStringForCSV(testCase?.test_case_name || result.test_name),
        '優先度': testCase?.priority || result.priority || 'Medium',
        '前提条件': this.cleanStringForCSV(testCase?.preconditions || ''),
        'テスト手順': this.formatTestSteps(testCase?.test_steps),
        '期待結果': this.cleanStringForCSV(testCase?.expected_result || ''),
        '実行結果': this.cleanStringForCSV(result.actual_result || ''),
  ...
  formatTestSteps(steps) {
    ...
    return steps.map(step => {
      // アスタリスクを削除してクリーンな文字列にする
      const cleanDescription = (step.description || '').replace(/\*+/g, '').trim();
      return `${step.step_number}. ${cleanDescription}`;
    }).join(' → ');
  }
  
  cleanStringForCSV(str) {
    if (!str) return '';
    // アスタリスクと余分な記号を削除
