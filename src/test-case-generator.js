...
import { ECCubeAnalyzer, EC_CUBE_TEST_DATA } from './eccube-analyzer.js';
...
class TestCaseGenerator {
  ...
  async startRecording() {
    ...
    try {
      ...
      // playwright.config.jsの設定を適用
      const defaultArgs = ['--start-maximized', '--lang=ja-JP', '--font-render-hinting=none'];
      ...
      // 初期URL設定
      const initialUrl = this.options.url || testInfo.custom_url || target.base_url;
      ...
      // EC-CUBE検出と解析
      try {
        const pageType = await this.eccubeAnalyzer.detectPageType(this.page);
        if (pageType !== this.eccubeAnalyzer.pageTypes.UNKNOWN) {
          console.log(chalk.magenta(`\n🛒 EC-CUBE ${pageType} ページを検出しました`));
          const eccubeData = await this.eccubeAnalyzer.analyzePage(this.page);
          
          // EC-CUBE用のテストシナリオを提案
          if (eccubeData.scenarios && eccubeData.scenarios.length > 0) {
            console.log(chalk.cyan('\n推奨テストシナリオ:'));
            eccubeData.scenarios.forEach((scenario, index) => {
          ...
          // テストケース情報にEC-CUBEデータを追加
          testInfo.eccube_data = eccubeData;
        }
      } catch (error) {
        console.log(chalk.gray('EC-CUBE検出をスキップ'));
      }
  ...
  setupRecordingListeners() {
    ...
    this.page.on('fill', async (element, value) => {
      try {
        ...
        // EC-CUBEフィールドの場合、適切なテストデータを生成
        let finalValue = value;
        if (this.testInfo && this.testInfo.eccube_data) {
          finalValue = this.eccubeAnalyzer.generateECCubeTestData(name, element.type) || value;
        }
        ...
        this.recordedSteps.push({
          ...
          target: selector,
          value: finalValue,
          description: `${placeholder || name || selector}に"${finalValue}"を入力する`
        });
        
        console.log(chalk.gray(`  [記録] 入力: ${selector} = "${finalValue}"`));
      } catch (error) {
        // エラーは静かに処理
      }
    });
  ...
  async getTestCaseInfo() {
    ...
    const answers = await inquirer.prompt([
      ...
      },
      {
        type: 'input',
        name: 'custom_url',
        message: '開始URL (デフォルトURLから変更する場合、Enterでスキップ):',
        default: ''
      }
    ]);
