  ...
  generateTestCasesFromElements(elements, url) {
    ...
    elements.links.forEach(link => {
      ...
      const isValidLink = href && 
                         ...
                         (link.text?.trim() || link.accessibleName?.trim());
      ...
      if (isValidLink) {
        testCases.push({
          ...
          action: 'click',
          expected_result: `リンクがクリックされ、${link.href || '指定されたページ'}に遷移する`
        });
      }
    });
    ...
    elements.buttons.forEach(button => {
      // 無効なボタンを除外
      const hasText = button.text?.trim() || button.accessibleName?.trim();
      const isValidButton = hasText && !button.disabled;
      
      if (isValidButton) {
        testCases.push({
          test_id: `TC${String(testIdCounter++).padStart(3, '0')}`,
          test_case_name: `ボタン_${button.accessibleName || button.text || 'ボタン'}クリック`,
          test_category: 'ボタン',
