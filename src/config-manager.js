...
class ConfigManager {
  ...
  loadConfig() {
    try {
      if (fs.existsSync(this.configPath)) {
        ...
        console.log(chalk.gray(`✅ 設定ファイルを読み込みました: ${this.configPath}`));
        return config;
      }
      console.log(chalk.yellow('⚠️  設定ファイルが見つかりません。デフォルト設定を使用します。'));
      console.log(chalk.gray(`  検索パス: ${this.configPath}`));
  ...
  getTestTarget(name) {
    if (!this.config.test_targets || this.config.test_targets.length === 0) {
      console.error(chalk.red('❌ test_targetsが設定されていません'));
      return null;
    }
    
    if (!name) {
      const target = this.config.test_targets[0];
      console.log(chalk.gray(`  デフォルトターゲット使用: ${target.name}`));
      return target;
    }
    
    const target = this.config.test_targets.find(t => t.name === name);
    if (target) {
      console.log(chalk.gray(`  指定ターゲット使用: ${target.name}`));
      return target;
    }
    
    console.log(chalk.yellow(`⚠️  "${name}"が見つからないため、デフォルトターゲットを使用`));
